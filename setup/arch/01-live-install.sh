...
pacstrap -K /mnt base linux linux-firmware intel-ucode \
  networkmanager niri ghostty efibootmgr \
...
echo "Downloading chroot setup script..."
curl -L -o /mnt/02-chroot-system.sh "${DOTFILES_REPO}/raw/branch/${DOTFILES_BRANCH}/setup/arch/02-chroot-system.sh"
chmod +x /mnt/02-chroot-system.sh

arch-chroot /mnt /02-chroot-system.sh

rm /mnt/02-chroot-system.sh
rm /mnt/setup_env.sh
rm /mnt/config.env
